...
import { baseUrl, axiosHeaders } from '../../../src/utils/constants';
import './Account.css';
...
const Account = () => {
    ...
    useEffect(async () => {
        const uriaccount = `${baseUrl}account/${userId}`;

        console.log(uriaccount)
        axios.get(uriaccount, axiosHeaders)
            .then(result => {
                setUser(user => result.data.user);
                setCategory(category => result.data.category);
                console.log(result.data.category, result.data.user)
            })
            .catch(error => {
